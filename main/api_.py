...
from main.http_common import format_numbers10, format_numbers_strong, format_numbers, format_numbers4
import logging
logger = logging.getLogger(__name__)
from main.models import dictfetchall, to_prec
...
def make_auto_trade(Order, TradePair, Price, Currency1, Sum1, Currency2, Sum2):
    ...
    if    int(TradePair.currency_on.id) ==  int(Currency1.id):
        Query = "SELECT * FROM main_orders  WHERE  currency1_id=%i AND currency2_id=%i \
                          AND status='processing' AND price >= %f  \
    ...
    ResultSum  = finish_create_order(TradePair, AccumSum2Buy, Order)    
...
def sell(Req, Trade_pair):   
                ...
                if TradePair.min_trade_base > Count*Price:
...
def ask(Req, UrlTitle):
    CurrentTradePair = TradePairs.objects.get(url_title = Pair)
    try: 
        Amount = Req.REQUEST.get("amount", None)
    except :
